...
if [ "$(uname)" == "Darwin" ]; then
  ...
  AR="/usr/local/opt/llvm/bin/llvm-ar" CC="/usr/local/opt/llvm/bin/clang" cargo build --bin example-common --target $TARGET --release
else
  cargo build --bin example-common --target $TARGET --release
fi
...
if [ "$STATUS" -eq "0" ]; then
      ...
      ./target/$TARGET/release/example-common.wasm \
      -o ./target/$TARGET/release/example-common.wasm
  true
else
